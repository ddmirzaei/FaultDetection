...
clear
%% Function f
f = piecewise(@(x,y) x.^2+y.^2<=0.5,@(x,y) sqrt(4-x.^2-y.^2)-(sqrt(7/2)),...
         @(x,y) (x-1).^2+(y-1).^2<=0.5,@(x,y) sqrt(4-(x-1).^2-(y-1).^2)-(sqrt(7/2)),0); 
...
%% Plots
disp('Plotting ...')
figure('Name','Example 6','NumberTitle','off')  
...
% narrowed points
figure('Name','Example 6','NumberTitle','off'); hold on

...
clear
%% Function f
f = piecewise(@(x,y) y>=(5/3)*x.^2-(11/3)*x+2.2 ,@(x,y) x.^2+x+sin(2*y),...
              @(x,y)(y<(5/3)*x.^2-(11/3)*x+2.2)&(y<=x+0.01),@(x,y) 0,@(x,y) (x+y).*y.^2 -1.2 );
...
% narrowed points
figure('Name','Example 2','NumberTitle','off'); hold on
...
figure('Name','Example 2','NumberTitle','off') ; hold on
gam1 = @(x,y) y-(5/3)*x.^2+(11/3)*x-2.2;
gam2 = @(x,y) y-x-0.01;
p3 = fimplicit(gam1,[0 1 0 1],'k--');
fimplicit(gam2,[0 .6 0 .6],'k--');

...
clear
%% Function f
f = piecewise(@(x,y)(x-0.4-0.1*sin(2*pi*y))> 0, @(x,y) abs(x.^2+y.^2-0.5)-x+y.^2, ...
              @(x,y)(x-0.4-0.1*sin(2*pi*y))<=0,@(x,y) abs(x.^2+y.^2-0.5)-x+y.^2+0.3);
...
%% Plots
disp('Plotting ...')
figure('Name','Example 5','NumberTitle','off')  
...
% narrowed points
figure('Name','Example 5','NumberTitle','off'); hold on
